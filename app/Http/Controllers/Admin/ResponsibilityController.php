...
use App\Models\{Activity, ActivityLog, Competencies, Goal, GoalCategory, GoalStatus, KeyResult, Responsibility, ReviewCycle, User};
use App\Models\Role;
use DataTables;
...
class ResponsibilityController extends Controller
{
    ...
    public function index(): View
    {
        ...
        $data['sub_title'] = 'View';
        return view('admin.goal.responsibility.responsibility', $data);
    }
    ...
    public function list(Request $request)
    {
        ...
        if ($request->ajax()) {
            $data = Responsibility::with('user')->get();
            ...
            <div class="progress-bar" role="progressbar" style="width: ' . $row->total_progress . '%;" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100">' . $row->total_progress . '%' . '</div>
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $editUrl = route('admin.updateresponsibility', $row->id);
                    ...
                    $action .= '<button type="button" title="Activity" onclick="activities(\'' . $row->id . '\')" class="btn-sm btn btn-outline-dark waves-effect waves-light mr-1 mb-0"><i class="fadeIn animated bx bx-history"></i></button>';
                    $action .= '</div>';
                    return $action;
                })
    ...
    public function create(Request $request): View
    {
        ...
        $data['users'] = User::get();
        return view('admin.goal.responsibility.create', $data);
    }
    ...
    public function update($id)
    {
        ...
        $data['title'] = 'Add Responsibility';
        $data['sub_title'] = 'Create';
        $data['responsibilty'] = Responsibility::with('keyresult')->find($id);
        $data['users'] = User::get();
        // echo "<pre>"; print_r($data['competencies']->toArray()); die;
        return view('admin.goal.responsibility.update', $data);
    }
    ...
    public function updateKeyHistory($table, $requestData)
    {
        ...
        $createActitive = Activity::create([
            ...
            'is_admin' => 1,
            'responsibilities_id' => $table->responsibilities_id,
    ...
    public function store(Request $request)
    {
        ...
        if ($request->id) {
            ...
            $data = Responsibility::with('user')->find($request->id);
            ...
            foreach ($keyresults as $keyresult) {
                $keyresult->responsibilities_id = $data->id;
                $keyresult->save();
            }

            $activities =  Activity::where('time', $request->time)->get();
            foreach ($activities as $activity) {
                $activity->responsibilities_id = $data->id;
                $activity->save();
            }
            ...
            $competencies =    Responsibility::with('user')->find($data->id);
    ...
    public function StoreresponsibilityKey(Request $request)
    {
        // echo "<pre>"; print_r($request->all()); die;
        if ($request->id) {
    ...
    {
        $activities = Activity::with(['activityLog', 'user.Image', 'admin'])->where('responsibilities_id', $request->id)->orderBy('created_at', 'desc')->get();
