...
class GoalController extends Controller
{
 ...
 public function list(Request $request)
            ...
            return DataTables::of($data)
                ...
                ->addColumn('progressbar', function ($row) {
                    ...
                    $progrebar = '<div class="progress mb-0">
                             <div class="progress-bar" role="progressbar" style="width: ' . number_format($row->totalprogressbar) . '%;" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100">' . number_format($row->totalprogressbar) . '%' . '</div>';
                    return $progrebar;
                })
    ...
    public function update(Request $request, $id): View
    {
        ...
        $data['goalcategories'] = GoalCategory::where('status', 1)->get();
        $data['reviewcycles'] = ReviewCycle::where('status', 1)->get();
        $data['keyresults'] = KeyResult::where('time', $goal->time)->get();
    ...
    public function store(Request $request)
    {
        ...
        if ($request->id) {
            ...
            $userid =  Auth::guard(name: 'web')->user()->id;
            ...
            $data->update([
                ...
                'time' => $request->time,
                'totalprogressbar' => number_format($request->totalkeypregressbar,2),
                'user_id' => $user->id,
                'type' => $type,
    ...
    public function goalOverView(Request $request)
    {
        ...
        foreach ($users as $user) {
            ...
            if ($request->ajax()) {
              ...
              return view('user.goal.teamoverview.teamoverview', compact('users', 'title','departments','reviewCycles'));
    }
    ...
    return view('user.goal.teamoverview.teampartialgoaloverview' , compact('users', 'title', 'departments','username','department','reviewCycles'));
    }
    ...
    {  
        ...
        return view('user.goal.hroverview.allgoaloverview', compact('users', 'title', 'departments', 'username', 'department', 'reviewCycles'));
    }  
    ...
    {     echo "<pre>";print_r($request->toArray());die;
        if($request->type == 0){
            $appraisalData=  Goal::find($request->appraisalId);
