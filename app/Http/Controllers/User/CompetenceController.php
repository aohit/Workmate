...
use App\Models\{Activity, ActivityLog, Competencies, Goal, GoalCategory, GoalStatus, KeyResult, ReviewCycle, User};
use App\Models\Role;
use DataTables;
...
class CompetenceController extends Controller
{
    ...
    public function indexs(Request $request): View
    {
        ...
        $data['comptencies'] = Competencies::with('keyresult')->where('user_id',$userid)->get();
        // echo "<pre>"; print_r($userid); die;
        return view('user.goal.completece.competencies', $data);
    }
    ...
    public function create(Request $request): View
    {
        ...
        $data['title'] = 'Add Competencies';
        $data['sub_title'] = 'Create';
         $data['userid'] = base64_decode($request->id);
        $data['competencies'] = Competencies::with('keyresult')->find(base64_decode($request->id));
        return view('user.goal.completece.create', $data);
    }
    ...
    public function update($id)
    {
        ...
        $data['title'] = 'Add Competencies';
        $data['sub_title'] = 'Create';
        $data['competencies'] = Competencies::with('keyresult')->find(base64_decode($id));
        // echo "<pre>"; print_r($data['competencies']); die;
        return view('user.goal.completece.update', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->id) {
            ...
            $data = Competencies::with('user')->find($request->id);
            ...
            foreach ($keyresults as $keyresult) {
                $keyresult->competencies_id = $data->id;
                $keyresult->save();
            }

            $activities =  Activity::where('time', $request->time)->get();
            foreach ($activities as $activity) {
                $activity->competencies_id = $data->id;
                $activity->save();
            }
            ...
            $competencies =    Competencies::with('user')->find($data->id);
    ...
    public function updateKeyHistory($table, $requestData)
    {
        ...
        $createActitive = Activity::create([
            'date' => date('Y-m-d'),
            'user_id' => Auth::guard('web')->user()->id,
            'competencies_id' => $table->competencies_id,
            'title' => "update this entry",
        ]);
    ...
    {
        // echo "<pre>"; print_r($request->all()); die;
        $data = Competencies::find($request->id); 
        $data->delete(); 
       $keies =  KeyResult::where('competencies_id',$request->id)->get();
        ...
        return response()->json([
            'success'=> 1,
            'message'=>"Competencies deleted successfully."
        ]);
    ...
    {
        $activities = Activity::with(['activityLog', 'user.Image', 'admin'])->where('competencies_id', $request->id)->orderBy('created_at', 'desc')->get();
