...
class AppraisalController extends Controller
{
    ...
    public function AppraisalList(Request $request)
    {
        if ($request->ajax()) {
            ...
            ->addColumn('name', function($row){
                ...
                ->rawColumns(['name','reviewcycle','deadline','action'])
                ->make(true);
        }
    }
    ...
    public function list(Request $request)
    {
        if ($request->ajax()) {
            ...
            // }else{
                $data = Appraisal::where('user_id',$user->id)->where('self_review',0)->get();
            // }
            // echo "<pre>"; print_r($data->toArray()); die;
            return DataTables::of($data)
            ->addIndexColumn()
    ...
    public function completedlist(Request $request)
    {
        if ($request->ajax()) {
            ...
            // }else{
                $data = Appraisal::where('user_id',$user->id)->where('self_review',1)->get();
            // }
            // echo "<pre>"; print_r($data->toArray()); die;
            return DataTables::of($data)
            ->addIndexColumn()
