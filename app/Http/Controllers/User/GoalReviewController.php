...
class GoalReviewController extends Controller
{
    ...
    { 
        if ($request->ajax()) {
            ...
            // echo "<pre>";print_r($user_id);die;
            $data = GoalReview::with(['user','Goals','reviewCycle','InputTypesData'])->where('employee_id', $user_id)->where('self_review', 0)->orderBy('updated_at','DESC')->get(); 
            return DataTables::of($data)
                ->addIndexColumn()
                ->addColumn('employee_id', function($row){
                    $user = User::where('id',$row->employee_id)->select('name')->first();
                ...
                ->rawColumns(['employee_id','input_type_id','review_cycle_id','rating_id','updated_at','action'])
                ->make(true);
    }
    }
    ...
    public function Completedlist(Request $request)
    {
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->rawColumns(['employee_id','input_type_id','manager_rating','review_cycle_id','rating_id','updated_at','action'])
                ->make(true);
    }
    }
    ...
    public function hrAddGoalReview()
    {
        $data['title'] = 'Goal Review';
        $data['sub_title'] = 'Goal Review'; 
        $data['employees'] = User::where('manager_id',Auth::guard('web')->user()->id)->get();
        $data['user']  = auth()->user();
        $data['types'] = InputType::get();
        $data['reviewcycles'] = ReviewCycle::where('status',1)->get();
        $data['ratings'] = RatingScale::with('ratingScaleOption')->get();
    ...
    public function employeelist(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $data = GoalReview::with(['user', 'Goals', 'reviewCycle', 'InputTypesData'])
    ...
    {  
        if ($request->ajax()) {
            ...
            if (auth('web')->user()->hasPermissionTo('hr-employee-goal-review')){
                ...
                ->where('self_review',1)->orderBy('updated_at','DESC')->get(); 
            }else{
                $data = GoalReview::with(['user','Goals','reviewCycle','InputTypesData'])->where('manager_id',$user_id)->where('self_review',1)->where('manager_review',0)->orderBy('updated_at','DESC')->get(); 
            }
