    ...
    <div class="card">
        <form id="formSubmit" onsubmit="formSubmit(this);return false;">
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="card-body">
                            <div class="row">
                                <input type="hidden" name="id" value="{{$competencies->id}}">
                                 <input type="hidden" name="userid" value="{{$competencies->user_id}}">
                                <div class="progress-container my-2">
                                    <div class="progress ">
                                        <div class="progress-bar text-dark" role="progressbar"
                                            style="width: {{ $competencies->total_progress }}%" id="totalkeyresult"
                                            aria-valuenow="100" aria-valuemin="0" aria-valuemax="100"></div>
                                    </div>
                                    <span id="totalkeyresultspan">{{ $competencies->total_progress }}%</span>
                                </div>
                                ...
                                <div class="mb-3 col-lg-6">
                                    ...
                                    <input type="text" class="form-control" id="validationCustom01" placeholder="Title"
                                        name="title" value="{{ $competencies->title }}" />
                                </div>
                                <div class="mb-3 col-lg-6">
                                    <label for="validationCustom02" class="form-label">Status</label>
                                    <select class="form-select" name="status">
                                        <option value="">-Select-</option>
                                        <option value="Active" @selected($competencies->status == 'Active')>Active</option>
                                        <option value="Archived" @selected($competencies->status == 'Archived')>Archived</option>
                                    </select>
                                </div>

                                <div class="mb-3 col-lg-12">
                                    <label for="validationCustom01" class="form-label">Description</label>
                                    <textarea type="text" class="form-control" id="validationCustom01" placeholder="Description..." name="description">{{ $competencies->discription }}</textarea>

                                </div>
                                <input type="hidden" id="timeData" name="time" value="{{ $competencies->time }}">
                                <div>
                            ...
                            <div class="row" id="progresKey">
                                <input type="hidden" id="totalkeies" value="{{ count($competencies->keyresult) }}">
                                @foreach ($competencies->keyresult as $results)
                                            ...
                                            <div class="progress-container">
                                                <div class="progress">
                                                    <div class="progress-bar" role="progressbar"
                                                        style="width: {{ $results->total_progress }}%;"
                                                        aria-valuenow="100" aria-valuemin="0" aria-valuemax="100"></div>
                                                </div>
                                            </div>
            ...
            <div class="row px-2 mb-2">
                <div class="col">
                     <a href="{{ route('goal',['id' => base64_encode($competencies->user_id)]) }}" class="btn btn-light">Close</a>
                </div>
...
@section('page-js-script')
    ...
    <script>
        ...
        function formSubmitkey(e) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('storecompetenciedskey') }}",
                data: formData,
                contentType: false,
                processData: false,
        ...
        function formSubmit(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success == 1) {
                        toastr.success(response.message, 'Success');
                         window.location.replace("{{ route('goal',['id' => base64_encode($competencies->user_id)]) }}");
                    } else {
                        toastr.error("Find some error", 'Error');
