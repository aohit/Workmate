                ...
                <div class="card-body">
                    <div class="row">
                        <div class="mb-3 col-lg-12">
                            <form action="{{ route('admin.leave.file.upload') }}" method="post" class="dropzone"
...
function ChangeLeaveTime(start, id) {
  ...
  function startDate(start) {
    ...
    function endDate(end) {
        ...
        var path = "{{ route('admin.leave.dates') }}";
...
<script type="text/javascript">
    ...
    function removeFile(fileId) {
        if (confirm('Are you sure you want to remove this file?')) {
            var path = "{{ route('admin.leave.file.remove') }}";
            var csrfToken = $('meta[name="csrf-token"]').attr('content');
            $.ajax({
                type: 'POST',
                url: path, // Adjust the URL as needed
