                        ...
                        <div class="row">
                            <div class="mb-3 col-lg-6">
                                ...
                                <input type="text" class="form-control" id="validationCustom01"
                                    placeholder="Holiday title" name="title" value="{{ $uinfo->title }}"/> 
                            </div>
                            <div class="mb-3 col-lg-6">
                                <label for="validationCustom04" class="form-label">Date</label>
                                <input type="text" name="date" class="form-control basic-datepicker"
                                    placeholder="Holiday Date" value="{{ $uinfo->date }}">
                            </div>
                            {{-- <div class="mb-3 col-lg-6">
                                <label for="validationCustom10" class="form-label">Country</label>
                                <select class="form-select select2"  name="country" >
                                    <option value="">Select Country...</option>
                                    @foreach ($countries as $country)
                                    <option value="{{$country->id}}" @if($uinfo->country == $country->id) @selected(true) @endif>{{ $country->name }}</option>
                                    @endforeach
                                  </select>
                            ...
                            <div class="mb-3 col-lg-12">
                                ...
                                <select class="form-select" name="status"> 
                                    ...
                                    <option value="0" @if($uinfo->status == 0) selected @endif>Deactivate</option> 
                                </select>
                            </div> 
                        </div>
                    {{-- </div> --}}
